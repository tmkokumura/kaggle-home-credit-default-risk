...
log_handler = FileHandler('../log/kernel_ann_antonova.py', 'a')
log_handler.setFormatter(log_fmt)
...
logger.info('writing: ../output/kernel_1_aggregate.csv')
df.to_csv('../output/kernel_1_aggregate.csv', index=False)
...
df = clean_data(df)
logger.info('writing: ../output/kernel_2_clearn.csv')
df.to_csv('../output/kernel_2_clearn.csv', index=False)
...
#####################################
feature_importance, scor = cv_scores(df, 5, lgbm_params, test_prediction_file_name='../result/submission_kernel_lgbm.csv')
...
# best_params
# bo.res['max']['max_val']
# feature_importance, scor = cv_scores(df, 5, best_params, test_prediction_file_name = '../result/submission_kernel_bayes.csv')
